            ...
            <table className="highlight responsive-table centered">
                ...
                <tbody>
                {
                    filteredData.map(eachRecord=>{
                        const formattedTimeStamp = formatDistanceToNow(new Date(eachRecord.timestamp), {
                            includeSeconds : true,
                            addSuffix : true
                        })
                        const newRecordWithFormattedTimeStamp = {...eachRecord, timestamp: formattedTimeStamp}
                        // Store the keys in a binding to prevent calling the Object.keys() method each time
                        const keysForRecordWithFormattedTimeStamp = Object.keys(newRecordWithFormattedTimeStamp);

                        return(
                            <tr key={newRecordWithFormattedTimeStamp.recordID}>
                                {
                                    keysForRecordWithFormattedTimeStamp.map(eachKey =>(
                                        <td key={eachKey}>{newRecordWithFormattedTimeStamp[eachKey]}</td>
                                    ))
                                }
                            </tr>
                        )
